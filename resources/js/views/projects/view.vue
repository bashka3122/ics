<template>
    <div>
        <title-bar :title-stack="['Admin', 'Projects', 'List']" />
        <hero-bar>
        ...
        <card-component class="has-table has-mobile-sort-spaced" icon="account-multiple">
            <card-toolbar>
                <router-link slot="left" to="/projects/new">
            ...
            <b-table :checked-rows.sync="checkedRows" :checkable="true" :loading="isLoading" :paginated="paginated"
                ...
                <b-table-column label="Funder" field="amount" sortable v-slot="props">
                    {{ props.row.funder }}
                </b-table-column>
                <b-table-column label="Agency" field="note" sortable v-slot="props">
                    {{ props.row.agency }}
                </b-table-column>


                <b-table-column label="Number of schools" field="note" sortable v-slot="props">
                    {{ props.row.schools }}
                </b-table-column>
                <b-table-column label="Number of teachers" field="note" sortable v-slot="props">
                    {{ props.row.teachers }}
                </b-table-column>

                <b-table-column custom-key="actions" class="is-actions-cell" v-slot="props">
                    <div class="buttons is-right">
                        <router-link :to="{ name: 'projects.edit', params: { id: props.row.id } }"
...
export default {
        ...
        filteredItems() {
            return this.projects.filter(item => {
                return item.name.toLowerCase().indexOf(this.search.toLowerCase()) > -1
                    || item.funder.toLowerCase().indexOf(this.search.toLowerCase()) > -1
    ...
    methods: {
        ...
        trashConfirm() {
            ...
            if (this.trashObject) {
                method = 'delete'
                url = `project/${this.trashObject.id}/destroy`
