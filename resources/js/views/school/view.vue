<template>
    <div>
        <title-bar :title-stack="['Admin', 'Schools', 'List']" />
        <hero-bar>
            Schools List
        ...
        <card-component class="has-table has-mobile-sort-spaced" icon="account-multiple">
            <card-toolbar>
                <router-link slot="left" to="/school/new">
            ...
            <b-table :checked-rows.sync="checkedRows" :checkable="true" :loading="isLoading" :paginated="paginated"
                ...
                <b-table-column label="District" field="amount" sortable v-slot="props">
                    {{ props.row.district }}
                </b-table-column>
                <b-table-column label="official" field="note" sortable v-slot="props">
                    {{ props.row.email }}
                </b-table-column>
                <b-table-column label="phone" field="note" sortable v-slot="props">
                    {{ props.row.no }}
                </b-table-column>
                <b-table-column label="Manager" field="note" sortable v-slot="props">
                    {{ props.row.manager }}
                </b-table-column>
                <b-table-column label="Number of manager" field="note" sortable v-slot="props">
                    {{ props.row.tell }}
                </b-table-column>

                <b-table-column custom-key="actions" class="is-actions-cell" v-slot="props">
                    <div class="buttons is-right">
                        <router-link :to="{ name: 'school.edit', params: { id: props.row.id } }"
...
export default {
        ...
        filteredItems() {
            return this.school.filter(item => {
                return item.name.toLowerCase().indexOf(this.search.toLowerCase()) > -1
                    ...
                    || item.manager.toLowerCase().indexOf(this.search.toLowerCase()) > -1
                    // || item.phone.toLowerCase().indexOf(this.search.toLowerCase()) > -1
                    || item.district.toLowerCase().indexOf(this.search.toLowerCase()) > -1
    ...
    methods: {
        ...
        trashConfirm() {
            ...
            if (this.trashObject) {
                method = 'delete'
                url = `emps/${this.trashObject.id}/destroy`
